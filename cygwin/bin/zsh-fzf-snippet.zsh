...
FZF_SNIPPET_BINDKEY=${FZF_SNIPPET_BINDKEY:-'^x^s'}
...
function fzf-snippet-selection() {
  ...
  fzf_command='fzf --query "$LBUFFER" --delimiter=: --prompt="Snippet> " --expect=ctrl-e $FZF_SNIPPET_EXTRA_ARG'
  ...
  if [[ $snippet != "" ]]; then
    if [[ $key == "ctrl-e" ]]; then
      ...
      snippet=$(echo $snippet | awk -F "\t" '{for(i=2;i<NF;i++){printf("%s%s",$i,OFS=" ")}print $NF}')
      RBUFFER=$snippet
      CURSOR=$#BUFFER
    fi
  fi
